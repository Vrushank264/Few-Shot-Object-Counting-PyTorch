...
def build_custom_exemplar_dataloader(cfg, training, distributed=False):
    ...
    normalize_fn = transforms.Normalize(mean=cfg["dataset"]["pixel_mean"], std=cfg["dataset"]["pixel_std"])
    ...
    if training:
        ...
        transform_fn = BaseTransform(
            cfg["dataset"]["input_size"], hflip, vflip, rotate, gamma, gray
        )
    else:
        transform_fn = BaseTransform(cfg["dataset"]["input_size"], False, False, False, False)

    exemplar_fn = ExemplarTransform(cfg["dataset"]["input_size"])

    if training and cfg.get("colorjitter", None):
        colorjitter_fn = RandomColorJitter.from_params(cfg["train"]["colorjitter"])
    else:
        colorjitter_fn = None
    ...
    if training:
      dataset = CustomDataset(
          cfg["dataset"]["train"]["img_dir"],
          cfg["dataset"]["train"]["density_dir"],
          cfg["dataset"]["train"]["meta_file"],
          cfg["dataset"]["exemplar"],
      ...
      dataset = CustomDataset(
          ...
          cfg["dataset"]["val"]["density_dir"],
          cfg["dataset"]["val"]["meta_file"],
          cfg["dataset"]["exemplar"],
          transform_fn=transform_fn,
          exemplar_fn=exemplar_fn,
          normalize_fn=normalize_fn,
...
class CustomDataset(BaseDataset):
    def __init__(
        ...
        for meta in exemplar_metas:
            fname = meta["filename"].split('_')[1]
            path = os.path.join(cfg_exemplar["img_dir"], fname)
            #print(path)
    ...
    def __getitem__(self, index):
        ...
        # read img
        img_name = meta["filename"].split('_')[1]
        ...
        height, width = image.shape[:2]
        # read density
        density_name = meta["density"].split('_')[1]
