...
class Visualizer(ABC):
    ...
    def apply_scoremap(self, image, scoremap, alpha=0.5):
        ...
        return (alpha * image + (1 - alpha) * scoremap).astype(np.uint8), scoremap
    ...
    def vis_result1(self, filename, resname, height, width, output):
        """
        filename: str
        image: tensor c x h x w
        """
        ...
        output = output.permute(1, 2, 0)  # c x h x w -> h x w x c
        output = output.cpu().detach().numpy()
        #output = cv2.resize(output, (width.item(), height.item()))
        #if self.activation_fn:
        output = self.activation_fn(output)
        #if self.normalization:
        output = (output - output.min()) / (output.max() - output.min() + 1e-7)
        if self.with_image:
